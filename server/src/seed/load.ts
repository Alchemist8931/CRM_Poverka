...
/* Межповерочный интервал живёт в самом справочнике прототипа (DEV_TYPES.mpi,
   пункт fe-forms): по нему свидетельство считает «действительно до». Здесь
   остаётся только носитель — вода или тепло. Заказчик подтверждает интервалы
   листом «Приборы» в справочнике (пункт be-import). */
const DEVICE_TYPE_KIND: Record<string, 'Вода' | 'Тепло'> = {
  'Бетар СХВ-15': 'Вода', 'Бетар СГВ-15': 'Вода', 'Ителма WFW20': 'Вода',
  'Пульсар М-15': 'Вода', 'Норма СВК-15': 'Вода', 'ТСК-7 (тепло)': 'Тепло',
};
...
export async function seedDemoData(
  ...
  try {
    ...
    await insertMany(db, 'device_types',
      ...
      DEV_TYPES.map((t, i) => {
        const kind = DEVICE_TYPE_KIND[t.v];
        if (!kind || !(t.mpi > 0)) throw new Error(`Не задан носитель или межповерочный интервал для типа «${t.v}»`);
        return [t.v, t.grsi, t.mpi, kind, i];
      }));
